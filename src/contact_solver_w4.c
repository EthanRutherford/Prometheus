...
#define B3_GATHER_LANES_W4( wide, lanes, field )                                                                                 \
	wide = b3SetW4( lanes[0]->field, lanes[1]->field, lanes[2]->field, lanes[3]->field )
...
void b3PushContacts_ConvexW4( b3SolverBlock block, b3StepContext* context )
{
	...
	for ( int wideIndex = block.startIndex; wideIndex < block.startIndex + block.count; ++wideIndex )
	{
		...
		for ( int pointIndex = 0; pointIndex < pointCount; ++pointIndex )
		{
			...
			b3FloatW4 s = b3AddW4( b3AddW4( normalSeparation, b3SubW4( b3DotW4( normalB, rB ), b3DotW4( normalA, rA ) ) ),
								   cp->baseSeparations );
...
void b3ApplyRestitution_ConvexW4( b3SolverBlock block, b3StepContext* context )
{
	...
	for ( int wideIndex = block.startIndex; wideIndex < block.startIndex + block.count; ++wideIndex )
	{
		...
		for ( int pointIndex = 0; pointIndex < pointCount; ++pointIndex )
		{
			...
			b3FloatW4 armed =
				b3AndW4( b3AndW4( restitutionMask, b3LessThanW4( cp->relativeVelocities, negRestitutionThreshold ) ),
			...
			b3FloatW4 approachImpulse =
				b3MinW4( b3MaxW4( b3NegW4( b3MulW4( normalMass, vn ) ), zeroW ), b3MaxW4( impulse, zeroW ) );
			b3FloatW4 allowance =
